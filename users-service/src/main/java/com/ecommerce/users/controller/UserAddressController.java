...
import com.ecommerce.users.service.UserAddressService;
...
public class UserAddressController {
     ...
     * @param token the JWT token
     * @param userId the ID of the user
     * @return Mono<ResponseEntity<User></User>> the created user address association
     */
    @Operation(summary = "Add a user address association")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "User address association created successfully"),
    ...
    @PostMapping("/{userId}")
    public Mono<ResponseEntity<User>> addUser(@RequestHeader(HttpHeaders.AUTHORIZATION) String token,
                                              @PathVariable Long userId) {
        ...
        ResponseEntity<UserDAO> userDetails = authClient.getUserDetails(userId);

        return userAddressService.addUser(userDetails);
    }
